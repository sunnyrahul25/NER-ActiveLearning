...
from src.models.modules.linear_crf_inferencer import LinearCRF
...
class FastLinearCRF(LinearCRF):
    ...
    def forward_unlabeled(
        ...
        first_mask[
            interleave_idxs, interleave_idxs, idxs.repeat(self.label_size)
    ...
    def viterbi_decode(
        ...
        for d in range(depth - 1, -1, -1):
            ...
            right_argmax_idxs = torch.gather(
                temp_argmax,
                2,
                current_argmax_idxs.unsqueeze(-1).expand_as(temp_argmax),
            )
        ...
        left_argmax_idxs = torch.gather(
            ...
            3,
            current_argmax_idxs.unsqueeze(-1).expand_as(argmax_idxs[:, :seq_len, 0, :, :, :]),
        )
...
if __name__ == "__main__":
    ...
    model = FastLinearCRF(label_size=len(labels), label2idx=label2idx, idx2labels=labels)
    seq_len = 80
    batch_size = 5
