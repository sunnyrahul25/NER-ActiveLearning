...
class LinearCRF(nn.Module):
        ...
        for prev_label in self.idx2labels:
            ...
            for next_label in self.idx2labels:
                ...
                if prev_label == "O" and (
                    ...
                    transition[
                        self.label2idx[prev_label],
                        self.label2idx[next_label],
                    ] = -10000.0
    ...
    def forward_unlabeled(
        ...
        """Calculate the scores with the forward algorithm. Basically calculating the normalization
        term.
        ...
        last_alpha = torch.gather(
            alpha,
            1,
            word_seq_lens.view(batch_size, 1, 1).expand(batch_size, 1, self.label_size) - 1,
        ...
        last_alpha = log_sum_exp_pytorch(last_alpha.view(batch_size, self.label_size, 1)).view(
            batch_size
        )

        # final score for the unlabeled network in this batch, with size: 1
        # sunny: z is in log space
    ...
    def backward(self, lstm_scores: torch.Tensor, word_seq_lens: torch.Tensor) -> torch.Tensor:
        ...
        ).expand(batch_size, seq_len, self.label_size, self.label_size) + lstm_scores.view(
            batch_size, seq_len, 1, self.label_size
        ).expand(
        ...
        last_beta = log_sum_exp_pytorch(last_beta.view(batch_size, self.label_size, 1)).view(
            batch_size
        )
    ...
    def calculate_all_scores(self, lstm_scores: torch.Tensor) -> torch.Tensor:
        ...
        Basically, compute the scores for each edges between labels at adjacent positions. This
        score is later be used for forward-backward inference.
    ...
    def viterbi_decode(
        ...
        idxRecord = torch.zeros(
            [batchSize, sentLength, self.label_size],
            dtype=torch.int64,
            device=curr_dev,
        )
        ...
        for distance2Last in range(sentLength - 1):
            ...
            decodeIdx[:, distance2Last + 1] = torch.gather(
                lastNIdxRecord,
                1,
                decodeIdx[:, distance2Last].view(batchSize, 1),
            ).view(batchSize)
